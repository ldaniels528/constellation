...
/** 
 * The ARC::RADIUS function
 * @author lawrence.daniels@gmail.com
 */
public class ArcRadiusFunction extends StructuredSelectionFunction {
	...
	public ArcRadiusFunction() {
		super( 
			"ARC", "RADIUS", 
			"images/commands/curve/arc-radius.png", 
			"docs/functions/curve/arc_radius.html", 
			STEPS 
		);
	}
	...
	private void createArc( final ApplicationController controller ) {
		...
		final ModelElement element = new CxModelElement( arc );
		...
		controller.setStatusMessage( format( "Created arc '%s'", element ) );
	}
	...
	private RenderableElement createTemporaryArcByRadius( final PointXY cp, final PointXY ep ) {
		...
		if( stepNo >= STEP_2 && stepNo < STEP_4 ) {
			// create the circle
			...
			// finally, add the circle, radius note, and center-point to HUD
			hud.addAll( circle, noteR, cp );
		}
		
		else if( stepNo >= STEP_4 ) {
			// create the circle
			final ArcXY arc = 
				ArcXY.createArcThruCenterAndEndPoints( cp, anglePointA, anglePointB );
		...
		if( stepNo >= STEP_3 ) {
			...
			// add the line & note to HUD
			hud.addAll( lineA, noteA, anglePointA );
		}
		...
		if( stepNo >= STEP_4 ) {
			...
			// add the line & note to HUD
			hud.addAll( lineB, noteB, anglePointB );
		}
